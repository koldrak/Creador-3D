 ...
 * - Click derecho/izquierdo o Enter: salir de modo recolocación y volver a cámara
...
public class BlockBuilder3D extends Application {
    ...
    private Node helpBar() {
        Label help = new Label(
                ...
                "Click derecho/izquierdo o Enter = salir de edición | Crear/Eliminar tipos en la izquierda");
    ...
    private void setupInputHandlers(Scene scene) {
        scene.setOnKeyPressed(e -> {
            keys.add(e.getCode());
            if (mode == Mode.BLOCK_EDIT && e.getCode() == KeyCode.ENTER) {
                mode = Mode.CAMERA;
                editingBlock = null;
                e.consume();
            }
        ...
        subScene3D.setOnMousePressed(e -> {
            if (e.getButton() == MouseButton.SECONDARY) {
                ...
                e.consume();
            } else if (e.getButton() == MouseButton.PRIMARY && mode == Mode.BLOCK_EDIT) {
                mode = Mode.CAMERA;
                editingBlock = null;
                e.consume();
            }
    ...
    // ----- Colocar bloque con PickResult desde la mirilla -----
    private void placeBlockFromCrosshair() {
        PickResult pr = subScene3D.pick(subScene3D.getWidth() / 2, subScene3D.getHeight() / 2);
        if (pr == null || pr.getIntersectedNode() == null) return;
        if (!(pr.getIntersectedNode() instanceof Box)) return;
        Box hitBox = (Box) pr.getIntersectedNode();

        Point3D normal = pr.getIntersectedNormal();
        if (normal == null) return;

        BlockType sel = blockList.getSelectionModel().getSelectedItem();
        if (sel == null) return;

        double hx = hitBox.getWidth() / 2.0;
        double hy = hitBox.getHeight() / 2.0;
        double hz = hitBox.getDepth() / 2.0;

        double nx = normal.getX();
        double ny = normal.getY();
        double nz = normal.getZ();

        double cx = hitBox.getTranslateX();
        double cy = hitBox.getTranslateY();
        double cz = hitBox.getTranslateZ();

        double newX = cx + nx * (hx + sel.sizeX / 2.0);
        double newY = cy + ny * (hy + sel.sizeY / 2.0);
        double newZ = cz + nz * (hz + sel.sizeZ / 2.0);
        ...
        BlockInstance bi = new BlockInstance(sel, box, newX, newY, newZ);
        bi.applyTransform();
        blocks.add(bi);
